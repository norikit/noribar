...
//
//  Signatures confirmed against (per the spike brief — "do not trust memory"):
...
/// Thin, lazily-bound facade over the private SkyLight surface this spike needs.
enum SLS {
    ...
    private static let _currentSpace = bind("SLSManagedDisplayGetCurrentSpace", as: Fn_ManagedDisplayGetCurrentSpace.self)
    ...
    private static let _spaceAddWindows = bind("SLSSpaceAddWindowsAndRemoveFromSpaces", as: Fn_SpaceAddWindowsAndRemoveFromSpaces.self)
    ...
    static func setSpaceAbsoluteLevel(_ sid: UInt64, _ level: Int32) { _spaceSetAbsoluteLevel?(mainConnectionID, sid, level) }
