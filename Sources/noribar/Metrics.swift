...
/// Tiny thread-safe accumulator for noribar's runtime measurements.
final class Metrics {
    ...
    func snapshot() -> String {
        ...
        guard !ticks.isEmpty else {
            let rss = String(format: "%.1f", Metrics.residentMemoryMB())
            return "metrics: no ticks yet; reloads=\(reloads); rss=\(rss)MB"
        }
        ...
        let mx = us(sorted[sorted.count - 1])
        let rss = String(format: "%.1f", Metrics.residentMemoryMB())
        return """
        metrics: ticks=\(ticks.count) mean=\(us(mean)) p50=\(us(p50)) p99=\(us(p99)) max=\(mx) \
        reloads=\(reloads) rss=\(rss)MB
        """
    }
}
